...
function formatSingleValue(item: unknown): string {
    if (Array.isArray(item)) {
        ...
        // For variable entries with fixed/stale/bounds: [index, value, fixed, stale, lb, ub]
        if (item.length >= 6) {
            const fixed = item[2] ? '🔒' : '';
            const lb = item[4] !== null ? `lb:${item[4]}` : '';
            const ub = item[5] !== null ? `ub:${item[5]}` : '';
            const bounds = [lb, ub].filter(Boolean).join(' ');
            return `${key || 'value'}: ${val} ${fixed} ${bounds}`.trim();
        }
