...
export default function ConfigView() {
    ...
    function updateConfigHandler() {
        ...
        if (Object.keys(updateConfig).length === 0) {
            ...
            vscode.postMessage({
                type: "error",
                content: "The extension config in react updateConfig is empty, please check the extension config. From file configView.tsx",
            })
            return;
        }

        // error handling for updateConfig when updateConfig has empty value report to extension
        (Object.keys(updateConfig) as Array<keyof IExtensionConfig>).forEach((key) => {
            if (!updateConfig[key]) {
                console.log(`The ${key} in react updateConfig is empty, please check the extension config. From file configView.tsx`);
                vscode.postMessage({
                    type: "error",
                    content: `The ${key} in react updateConfig is empty, please check the extension config. From file configView.tsx`,
                })
                return;
            }
        })
