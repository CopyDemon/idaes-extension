...
export default function RunFlowsheet(
    ...
    return (
        <section className="run-flowsheet-section">
            ...
            {extensionErrorLogs && extensionErrorLogs.length > 0 && !isRunningFlowsheet && (
                <div 
                    className={`${css.error_banner}`}
                    onClick={() => {
                        vscode.postMessage({ frontendInstruction: 'focus_view', target: 'mermaid' });
                        vscode.postMessage({ frontendInstruction: 'switch_sub_tab', tab_name: 'logs', sub_tab_name: 'error' });
                    }}
                >
                    <span>
                        ⚠ Running flowsheet has error, click to show logs view
                    </span>
                </div>
            )}
        </section>
